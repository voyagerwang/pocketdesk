...
本机网页：<http://localhost:8765>

手机地址：`http://<这台 Mac 的局域网 IP>:8765`。启动后可用下面的命令查看 Wi-Fi 地址：

```bash
ipconfig getifaddr en0
```

例如显示 `192.168.1.23`，手机打开 `http://192.168.1.23:8765`。手机与 Mac 必须在同一 Wi-Fi，且不能处于访客网络/客户端隔离网络。
...
- 服务默认监听局域网，**没有配对或身份验证**；仅在受信任网络运行，切勿向公网转发 8765 端口。
