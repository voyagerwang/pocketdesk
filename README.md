...
安卓 Chrome 与 iPhone Safari 均可在原手机网页中点击发送工具栏的「画面」，打开全屏查看器自动获取 Mac 画面；「返回操控」回到输入和触控板，保留草稿。多屏才显示选择器。安卓实机兼容性仍待验证；无需安装手机 App。「放大画面」按钮以画面原尺寸展示，横向/纵向滚动阅读。

画面功能需要 macOS 14+；macOS 13 保留原有输入功能。未授权时点击「前往 Mac 授权」，在系统设置 → 隐私与安全性 → 屏幕与系统音频录制中允许 PocketDesk，必要时退出后重新启动应用，再刷新手机网页。

打开查看器后默认自动更新，可随时暂停；每帧完成约一秒再请求下一帧，不适合精细拖动。返回操控、Esc、浏览器返回或切后台时停止请求并释放页面画面；重新打开获取新画面。断网或权限错误停止自动刷新，恢复后手动刷新。时间标签只代表截图更新时间，不表示软件已回复完毕。电脑需保持唤醒并处于可操作的桌面会话。

`GET /api/screen/displays` 列出显示器，`GET /api/screen/frame?display=<id>` 返回最长宽度 1920 像素 JPEG，`POST /api/screen/permission` 请求 Mac 原生授权。全部画面端点沿用 Bearer 配对鉴权（本机回环豁免），不缓存、不落盘；显示整个所选屏幕，请注意该屏幕上的其他窗口。传输沿用当前局域网 HTTP，配对鉴权不等于传输加密。
