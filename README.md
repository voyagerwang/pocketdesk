...
电脑本机控制台右上角显示当前在线浏览器数量，悬停显示摘要，点击查看设备信息和连接记录。设备名称只能在电脑网页连接浮层中修改，名称保存在电脑上并用于后续连接和历史记录；手机仅自动展示系统、浏览器及浏览器实际提供的型号，iPhone 不推测具体型号，也不把显示名称当作系统原始名称。

在线状态基于已鉴权页面心跳，45 秒未收到心跳转离线；「控制中」额外通过服务端控制租约校验。后台被系统挂起可能显示离线，恢复后重新记录连接。记录保存在本机 `device-connections.json`，保留最近 30 天、最多 500 条，重启后不会把历史记录恢复为在线。历史时间是心跳观测时间，不是精确的物理网络断开时间。记录从此版本开始积累。

同一浏览器的标签页按一个设备合并；不同浏览器、地址来源或清除网站数据可能产生新的设备标识。设备描述由浏览器上报，仅供辨认，不作为安全身份。本次设备可见性功能不包含配对码有效期、撤销或传输安全改造。
