...
运行 `zsh scripts/setup-secure-channel.sh` 生成本机 TLS 身份，重启应用后控制台显示 HTTPS 二维码。将 `~/Library/Application Support/VoiceDeck/tls/PocketDesk-CA.cer` 通过可信文件传输交给自己的手机，在系统证书设置中安装为 CA，再扫描安全连接码。不得跳过浏览器证书警告；IP 更换或证书过期需要重新签发，脚本不会覆盖已有身份。
