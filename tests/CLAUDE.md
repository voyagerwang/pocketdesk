...
image-paste-policy.test.swift: 隔离验证 UU 文字在 Cmd+V 前等待新剪贴板同步、Chrome 多图逐张粘贴的剪贴板稳定/网页消费等待、只在非末张后要求插入点点击，以及 Group/StaticText 焦点向上归一到 WebArea、传统输入框取自身、浏览器工具栏拒绝；测试本身无剪贴板、鼠标与按键副作用。
