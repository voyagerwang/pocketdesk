...
ScreenCapture.swift: ScreenCaptureKit 单帧读取边界，macOS 14+ 枚举显示器并生成含鼠标的内存 JPEG；只按鉴权请求采集，无持续录屏或落盘。 (Fix icon flicker with long cache and add screen viewer entry)
