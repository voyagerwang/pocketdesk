...
Util.swift: 无状态渲染工具层——主局域网 IP 探测、稳定 .local 主机名、QR PNG、应用图标 PNG 提取；Server 的地址与图标端点调用它。
InputExecutor.swift: 键盘输入执行层，串行队列跑 activate → 粘贴 → Unicode → Return 注入序列，含图片预上传暂存与快捷键组合注入；与 PointerExecutor 平行。
PointerExecutor.swift: 指针执行层，虚拟光标 + 双屏包围盒钳制，move/drag/click/scroll/zoom 手势到 CGEvent 的映射；仅被 WSServer 消费。
Server.swift: HTTP 传输层 :46387，全部端点路由与静态页面服务，只翻译协议不做系统调用，写操作委托 InputExecutor；与 WSServer 平行。
WSServer.swift: 触控板传输层 :46388，WebSocket 监听、会话重置与消息转发到 PointerExecutor。
