 ...
 * [OUTPUT]: 提供接收者渲染、显式应用选择、小精灵往返与只读前台跟随；启动沿用电脑前台。
 * [POS]: 手机接收者路由层；显式应用选择才激活电脑，小精灵和跟随入口不操作桌面；未发送草稿阻止被动换目标。
...
function markSelected() {
  ...
  for (const button of document.querySelectorAll('[data-recipient-toggle]')) {
    button.textContent = isSprite ? '跟随电脑' : '回到小精灵';
  }
...
async function resumeFrontmost() {
  // “跟随”不意味着把已有指令搬进电脑；保留正文并说明恢复条件。
  if (hasRecipientDraft()) {
    message('草稿已保留，请先发送或清空，再跟随电脑。', 'warn');
    return;
  }
  const generation = ++selectGeneration;
  try {
    const response = await fetch('/api/status', { headers: authHeaders() });
    if (!response.ok) throw new Error('暂时无法识别电脑前台，请稍后再试。');
    const status = await response.json();
    if (generation !== selectGeneration || hasRecipientDraft()) return;
    if (!status.frontmostId && !status.frontmostName) throw new Error('暂时未识别到前台应用，请在电脑打开目标软件后再试。');
    lastActivateAt = 0;
    manualUntil = 0;
    applyFrontmost(status);
    message('已恢复跟随电脑前台。');
  } catch (error) {
    if (generation === selectGeneration) message(error.message, true);
  }
}

for (const button of document.querySelectorAll('[data-recipient-toggle]')) {
  button.addEventListener('click', () => {
    if (selected === SPRITE_ID) resumeFrontmost();
    else selectSprite();
  });
}
