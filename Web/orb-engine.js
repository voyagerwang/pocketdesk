/*
 * [INPUT]: 工作台 emotion-ball 原版 engine 模块；依赖顺序 rings → emotions → ball → engine。
 * [OUTPUT]: 原版全局表情能力，保留几何/配置/物理动画；engine 仅增加约 30fps 绘制上限。
 * [POS]: 手机球球的第三方实现；来源与完整许可见 ../docs/orb-attribution.md，不按项目 MIT 授权。
...
(function () {
  ...
  var ticker = {
    ...
    loop: function (now) {
      ticker.raf = 0;
      ticker.set.forEach(function (e) { if (!e._mobileFrameAt || now - e._mobileFrameAt >= 32) { e._mobileFrameAt = now; e._tick(now); } });
